 ...
 * \brief Matrix definition and accessers depending on the data layout
 ===========================================*/
    ...
    #define MATRIX_DIAG_MAJ_ALLOCATE(type, X, Y, name, initVal)     ALLOCATE_2D_ARRAY(type, 1+X+Y, 1+X+Y, name, initVal);    // The correct formula is sqrt(x*x + Y*Y).   But we use the triangular inequality to reduce the number of computations (and cause we target platforms with no sqrt func)
 ...
 * \brief Matrix (stencil) definition and accessors depending on the data layout
 ===========================================*/
    ...
    #define MATRIX_STENCIL_LINE_MAJ_ALLOCATE(type, X, Y, name, initVal)         MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_ALLOCATE(type, 3*X, Y, X_LINE, Y_LINE, name, initVal)
    #define MATRIX_STENCIL_LINE_MAJ_GET(m, x, y)                                MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_GET(m, 3*x,    y,   X_LINE, Y_LINE)
    #define MATRIX_STENCIL_LINE_MAJ_SET(m, x, y, val)                           MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_SET(m, 3*x,    y,   X_LINE, Y_LINE,    val);       \
                                                                                MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_SET(m, 3*x+2,  y-1, X_LINE, Y_LINE,    val);       \
                                                                                MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_SET(m, 3*x+1,  y+1, X_LINE, Y_LINE,    val)
    #define MATRIX_STENCIL_LINE_MAJ_ADD(m, x, y, val)                           {                                                                                   \
                                                                                    float tmp = val + MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_GET(m, 3*x, y);              \
                                                                                    MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_ADD(m, 3*x,      y,   X_LINE, Y_LINE, tmp);    \
                                                                                    MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_ADD(m, 3*x+2,    y-1, X_LINE, Y_LINE, tmp);    \
                                                                                    MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_ADD(m, 3*x+1,    y+1, X_LINE, Y_LINE, tmp);    \
                                                                                }
    ...
    #define MATRIX_STENCIL_LINE_MAJ_MAP_REDUCE_SUM_3(m, x, y)                   {                                                                                       \
                                                                                    float tmp   = MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_GET(m, (3*(x-1)), y, X_LINE, Y_LINE) \
                                                                                                + MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_GET(m, (3*x),     y, X_LINE, Y_LINE) \
                                                                                                + MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_GET(m, (3*x + 1), y, X_LINE, Y_LINE) \
                                                                                                + MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_GET(m, (3*x + 2), y, X_LINE, Y_LINE) \
                                                                                                + MATRIX_LINE_MAJ_LARGE_BLOCK_LINE_GET(m, (3*x + 3), y, X_LINE, Y_LINE) ;\
